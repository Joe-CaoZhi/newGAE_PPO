...
**我们的核心实验发现**（5 个种子，500K 步，Optimal PPO 基线，**确定性评估**）是 **HCGAE v2 同时在三个主要 MuJoCo 基准上实现净正收益**——这是首个做到这一点的 GAE 校正方法：
- **Hopper-v4**：比 Optimal PPO **+10.1%**（$1760\pm170$ vs. $1598\pm67$，$p$=0.42，$d$=+0.56，n.s.），Critic 收敛速度加快约 **47%**
- **Walker2d-v4**：比 Optimal PPO **+25.2%**（$1998\pm351$ vs. $1596\pm187$，$p$=0.42，$d$=+0.64，n.s.）
- **HalfCheetah-v4**：比 Optimal PPO **+4.3%**（$1550\pm174$ vs. $1487\pm27$，$p$=0.69，$d$=+0.23，n.s.），逆转了朴素 MC 校正带来的 **−16.0% 惩罚**——完全依赖于 EV 增长率门控
...
朴素 HCGAE v1 在 HalfCheetah-v4（密集奖励）上比 Optimal PPO 下降 **−16.0%**（1250 vs. 1487）——SCR 框架预测的失效模式得到了清晰验证：MC 回报方差超过 Critic 偏差（SCR < 1），校正适得其反。**HCGAE v2 通过引入 EV 增长率门控，将此失效完全逆转为 +4.3%**——这是本文最重要的机制发现，首次在 GAE 框架中通过检测 Critic *收敛速率*（而非绝对水平）来自适应地控制 MC 修正。此外，v2 在三个任务上均实现净正收益（Hopper +10.1%，Walker2d +25.2%，HalfCheetah +4.3%），实现了单一超参数组合的跨环境净正收益。
...
4. **解耦实验**（§4.6.1）：系统比较 HCGAE v2 在技巧禁用消融实验基线（无观测归一化、无优势归一化、无学习率退火）与 Optimal PPO 基线上的表现，揭示**环境依赖的耦合现象**——HCGAE v2 的增益在 Hopper-v4 上与 Optimal 技巧基本无关（+7.6% vs. +10.1%），但在 Walker2d-v4（−0.6% vs. +25.2%）和 HalfCheetah-v4（−31.6% vs. +4.3%）上强耦合。该发现表明观测归一化创造了 EV 驱动门控按预期工作的条件。
...
**统一训练协议（所有方法统一）。** 2 层 MLP（hidden=256），Adam 优化器（lr\_actor=3e-4，lr\_critic=1e-3），rollout 长度 2048，10 次更新 epoch，mini-batch 大小 64，γ=0.99，λ=0.95，clip ε=0.2，**无值函数裁剪**。所有方法均采用**观测归一化**（在线更新的运行均值/标准差）和 **per-minibatch 优势归一化**作为标准配置，与 Andrychowicz 等（2021）和 Huang 等（2022）CleanRL 最佳实践对齐。评测：每 20,480 步进行 10 次确定性 episode 评测；最终性能 = 最后 5 次评测的均值。
...
**参与比较的算法（主要 §4.2）：**
- **Standard PPO**：PPO（Schulman 等，2017），配备观测归一化、per-minibatch 优势归一化与 256×256 MLP——与 Huang 等（2022）CleanRL 标准最佳实践实现对齐
- **Optimal PPO**：Standard PPO + 线性学习率退火至 0 + 单一共享 Adam 优化器（Andrychowicz 等，2021 最佳实践）
...
为验证 HCGAE v2 的改进可归因于 GAE 校正本身、而非与实现技巧（观测归一化、per-minibatch 优势归一化、学习率退火）的耦合，我们在一个**技巧禁用消融实验基线**（三种技巧全部关闭）上运行配对实验，使用相同的 OptimalPPO 代码库，设置 `use_obs_norm=False`、`use_adv_norm=False`、`use_lr_anneal=False`。注：此消融实验有意关闭默认最佳实践设置，目的是单独分离 HCGAE 机制本身的贡献：
