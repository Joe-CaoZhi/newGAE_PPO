...
HCGAE v4 完备对照实验
====================
与 ICMLExperiment 完全对齐：
  - 相同超参、相同 seeds (0-4)
  - 3 environments × 5 seeds × 500K steps
  - 确定性评估 (dist.mean)
  - 对比：Optimal_PPO（已有，SKIP），Optimal_HCGAE_v2（已有，SKIP），Optimal_HCGAE_v4（新）

结果保存到 results/V4FullExperiment/
允许 SKIP 已存在结果（与 ICMLExperiment 数据对齐分析）
"""
import json
import os
import sys
...
SEEDS       = list(range(5))     # 0-4, same as ICMLExperiment
TOTAL_STEPS = 500_000
EVAL_FREQ   = 10_240
N_EVAL_EPS  = 10
...
if __name__ == "__main__":
    ...
    print(f"V4 Full Experiment: {envs} × {algos} × seeds={seeds} × {TOTAL_STEPS//1000}K steps")
    ...
    for env_name in envs:
        ...
        for algo in algos:
            vals = []
            for s in seeds:
                # Try local first, then ICML
                p = RESULTS_DIR / env_name / algo / f"{algo}_s{s}.json"
                if not p.exists():
                    p = icml_dir / env_name / algo / f"{algo}_s{s}.json"
                if p.exists():
                    d = json.load(open(p))
